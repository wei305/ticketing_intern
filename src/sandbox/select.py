...
from selenium.webdriver.support.expected_conditions import alert_is_present, visibility_of_any_elements_located
...
def check_alert(driver):
    try:
        alert = WebDriverWait(driver, 1).until(alert_is_present())
        sleep(1)
        alert.accept()
...
def enter_captcha(driver):
    ...
    cond = visibility_of_any_elements_located((By.CSS_SELECTOR, selector))
    try:
